...
const UserProductAdd = ({ usedPurchases, isLoadingProducts }) => {
  ...
  const userProductsPurchase = useUserProductsPurchaseActions({
  ...
  const onAddingProducts = () => {
    ...
    userProductsPurchase.mutate({ action: 'add', id, data });
  };
      ...
      <div className={styles.save_courses_button_wrapper}>
        ...
        {selectedOption?.length > 0 && (
          <button
            ...
            disabled={userProductsPurchase.isPending}
          >
            {userProductsPurchase.isPending ? (
              <ProgressSpinner />
            ) : (
