...
const deleteGifts = async (id, token) => {
...
const hideGifts = async (id, token, status) => {
...
const EditButton = ({ id, name, status }) => {
  ...
  const mutation = useMutation({
    mutationFn: async ({ action }) => {
      if (action === 'delete') await deleteGifts(id, token.accessToken);
      if (action === 'hide')
        await hideGifts(id, token.accessToken, status === HIDDEN ? PUBLISHED : HIDDEN);
    },
    onSuccess: () => {
